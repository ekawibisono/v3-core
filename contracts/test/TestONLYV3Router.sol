...
contract TestONLYV3Router is IONLYV3SwapCallback {
    ...
    function ONLYV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IONLYV3Pool(msg.sender).token0() : IONLYV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IONLYV3Pool(pools[0]).token1();
            IONLYV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IONLYV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IONLYV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
