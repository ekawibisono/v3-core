...
import { ONLYV3PoolSwapTest } from '../typechain/ONLYV3PoolSwapTest'
...
describe('ONLYV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('ONLYV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as ONLYV3PoolSwapTest
